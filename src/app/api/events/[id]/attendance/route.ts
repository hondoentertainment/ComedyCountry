...
export async function GET(
  ...
  try {
    const rl = await checkRateLimit(`events-attendance:${getRateLimitKey(request)}`, { limit: 60, windowSeconds: 60 });
    if (!rl.success) {
    ...
    logger.requestError(request, "Failed to get attendance", err);
...
export async function POST(
  ...
  try {
    ...
    logger.requestError(request, "Failed to update attendance", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
